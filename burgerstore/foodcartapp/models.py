...
class RestaurantQueryset(models.QuerySet):
    ...
    def with_geo_attributes(self):
        ...
        for restaurant in self:
            restaurant.geodata = next((filter(lambda geo: geo.address == restaurant.address, geo_attrs)))
...
class OrderQuerySet(models.QuerySet):
    ...
    def with_geo_attributes(self):
        ...
        geo_attrs = GeoData.objects.filter(address__in=addresses)
        for order in self:
            order.geodata = next((filter(lambda geo: geo.address == order.address, geo_attrs)))
